...
speedTest.showMarkers = function() {
    ...
    // insert a sort button and a dropdown menu
       ...
       panel.innerHTML = panel.innerHTML + '<div style="margin-left: 12px"> <button class="btn btn-primary" onclick="speedTest.mysort()">Sort by </button> ' +
    ...
    for (var each_one in speedTest.pics) {
        ...
        title.innerHTML = '<div class="property-item table-list">' +
            ...
            '<div style="height: 170px;"><a href="property-view.php?pass_bd_id=' + building_id + '" class="hover-effect"> <img src="' + unit_feature_pic + '" alt="thumb"></a></div>' +
            ...
            '<thead><tr><th style="border-right-style:none; border-top-style:none; border-left-style:none;"><h2 class="property-title" style="margin: 0px"><a style="color: #FFFFFF" href="property-view.php?pass_bd_id=' + building_id + '">' + unit_number + '</a></h2></th>' +
            ...
            '<h3 style="color: #FFFFFF">' + unit_price + '</h3>' +
            ...
            '<tr><td>&nbsp;<i class="fa fa-bed"></i>&nbsp;&nbsp;<span>' + bedrooms +
            '</span> </td><td>&nbsp;<i class="fa fa-bath"></i>&nbsp;&nbsp;<span>'+ bathrooms +
            '</span> </td></tr><tr><td>&nbsp;<i class="fa fa-arrows-alt"></i>&nbsp;&nbsp;<span>' + square_feet+
            '</span> </td><td>&nbsp;<i class="fa fa-building-o"></i>&nbsp;&nbsp;<span>' + type +
            '</span> </td></tr>'+


            '<tr><td colspan="2">&nbsp;<i class="fa fa-home"></i>&nbsp;&nbsp;<span>' + titleText + '</span></td></tr>' +

            '<tr><td colspan="2">&nbsp;&nbsp;<i class="fa fa-map-marker"></i>&nbsp;&nbsp;<span>' + address + '</span></td></tr>' +


            '<tr><td colspan="2">&nbsp;&nbsp;<i class="fa fa-calendar"></i>&nbsp;&nbsp;<span>' + 'Available: ' + available_date + '</span></td></tr>' +
        ...
        if (buildings.indexOf(building_id) == "-1") {
            ...
            //speedTest.markers[0]
            var latLng2 = new google.maps.LatLng(speedTest.pics[i].latitude, speedTest.pics[i].longitude);
...
speedTest.markerClickFunction = function(pic, latlng) {
    ...
    return function(e) {
        ...
        var infoHtml = '<div class="infoBox">'+
            ...
            '<a href="property-view.php?pass_bd_id='+ building_id +'" tabindex="0">' +

            '<img src="'+ fileurl+'" class="" style="height: 100px">' +
            ...
            '<p class="map_list_spec"><span>'+ '<b>Location:</b>'+'<br>'+address+'</span></p>'+
...
speedTest.mysort = function() {
    ...
    switch (option) {
        case "date":
            //alert(option + "  " + order);
            if (order == "ascend") speedTest.pics.sort(sort_by('available_date', false, function (a) {
                if (a == null) return "2016";
                return a.toUpperCase();
            }));
            else speedTest.pics.sort(sort_by('available_date', true, function (a) {
                if (a == null) return "2016";
                return a.toUpperCase();
            }));
            break;
        case "price":
            if (order == "ascend") {
                speedTest.pics.sort(sort_by('unit_price', false, function(a){
                    return parseFloat(a.match(/\d+/))}));
            }
            else {
                speedTest.pics.sort(sort_by('unit_price', true, function(a){
                    return parseFloat(a.match(/\d+/))}));
            }
            break;
        case "bedrooms":
            //alert(option + "  " + order);
            if (order == "ascend") speedTest.pics.sort(sort_by('bedrooms', false, parseInt));
            else speedTest.pics.sort(sort_by('bedrooms', true, parseInt));
            break;
        case "address":
            //alert(option + "  " + order);
            if (order == "ascend") speedTest.pics.sort(sort_by('address', false, function (a) {
                return a.toUpperCase();
            }));
            else speedTest.pics.sort(sort_by('address', true, function (a) {
                return a.toUpperCase();
            }));
            break;
        case "surface":
            //alert(option + "  " + order);
            if (order == "ascend") speedTest.pics.sort(sort_by('square_feet', false, function(a){
                //console.log("price" + a + " "+ parseFloat(a.match(/\d+/)) +"\n");
                return parseFloat(a.match(/\d+/))}));
            else speedTest.pics.sort(sort_by('square_feet', true, function(a){
                //console.log("price" + a + " "+ parseFloat(a.match(/\d+/)) +"\n");
                return parseFloat(a.match(/\d+/))}));
            break;
        default:
