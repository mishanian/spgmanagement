...
class Payment {
	...
	public function get_deposit_info($deposit_id) {
		try {
			$this->crud->query("SELECT building_infos.building_name AS building_name, apartment_infos.unit_number AS unit_number,floor_infos.floor_name AS floor, tenant_id, payment_methods.name AS payment_method, amount, paid_date, deposits.comments, company_infos.name AS company_name,employee_infos.full_name AS proceed_by,deposit_status,deposit_types.name AS deposit_type,return_date,return_employee_id FROM deposits,building_infos,apartment_infos,payment_methods,employee_infos,floor_infos,company_infos,deposit_types WHERE building_infos.building_id = deposits.building_id AND apartment_infos.apartment_id = deposits.apartment_id AND payment_methods.id = deposits.payment_method_id AND employee_infos.employee_id = deposits.employee_id AND floor_infos.floor_id = apartment_infos.floor_id AND company_infos.id = employee_infos.company_id AND deposit_types.id = deposits.deposit_type_id AND deposits.id = :id ");
