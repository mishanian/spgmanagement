...
(function ($) {
  ...
  var map_full_btn = $('.map-next-prev-actions #houzez-gmap-full');
  ...
  if (property_menu.length) {
    ...
    $(window).on('scroll', function () {
      ...
      target_block.each(function () {
        ...
        if (
          scroll_top >=
          target_this.offset().top - topMargin - property_menu_height
        ) {
  ...
  if (price_range_advanced.length > 0) {
    ...
    min_price_range_hidden.val(
      '$ ' + addCommas(price_range_advanced.slider('values', 0))
    );

    max_price_range_hidden.val(
      '$ ' + addCommas(price_range_advanced.slider('values', 1))
    );

    min_price_range.text(
      '$ ' + addCommas(price_range_advanced.slider('values', 0))
    );

    max_price_range.text(
      '$ ' + addCommas(price_range_advanced.slider('values', 1))
    );
  }
  ...
  if (size_range_advanced.length > 0) {
    ...
    min_size_range_hidden.val(
      size_range_advanced.slider('values', 0) + ' sqft'
    );

    max_size_range_hidden.val(
      size_range_advanced.slider('values', 1) + ' sqft'
    );
  ...
  function this_sticky(ele) {
    ...
    var sticky_nav = ele
      .clone()
      .removeAttr('style')
      .removeClass('houzez-header-transparent');

    var sticky_wrap = $(sticky_nav)
      .wrap("<div class='sticky_nav'></div>")
      .parent()
      .addClass(get_header_class)
      .removeClass('houzez-header-transparent');

    sticky_wrap = sticky_wrap.removeClass(
      'header-transparent not-splash-header nav-left'
    );
  ...
  houzez_tabers('.houzez-tabs > tr', '.houzez-taber-body .tab-pane', 100);
  ...
  $(document).on('mouseup', function (e) {
    ...
    if (
      !tip.is(e.target) && // if the target of the click isn't the container...
  ...
  view_changer.on('click', function () {
    ...
    if (view_this.hasClass('btn-list')) {
      ...
      view_area
        ...
        .addClass('grid-view grid-view-3-col');
    }
  });
  ...
  function setSectionHeight() {
    ...
    var searchH =
      getWindowHeight() - splash_header_height - splash_footer_height;
    ...
    if (getWindowWidth() >= 992) {
      ...
      if (
        ...
        totalTopBarsHeight =
          parseInt(search_bar_height) + parseInt(header_height);
      }
      ...
      if (
        ...
        totalTopBarsHeight =
          ...
          parseInt(top_bar_height) +
          parseInt(search_bar_height);
      }

      if (header.length && admin_bar.length) {
        totalTopBarsHeight =
          parseInt(header_height) + parseInt(admin_bar_height);
      }

      if (header.length && admin_bar.length && top_bar.length) {
        totalTopBarsHeight =
          parseInt(header_height) +
          parseInt(admin_bar_height) +
          parseInt(top_bar_height);
      }
      ...
      if (
        ...
        totalTopBarsHeight =
          parseInt(mob_search_bar_height) + parseInt(mob_header_height);
      }
      ...
      if (header_mobile.length && top_bar.length) {
        totalTopBarsHeight =
          parseInt(mob_header_height) + parseInt(top_bar_height);
      }
      ...
      if (
        ...
        totalTopBarsHeight =
          parseInt(mob_header_height) +
          parseInt(top_bar_height) +
          parseInt(mob_search_bar_height);
      }

      if (header_mobile.length && admin_bar.length) {
        totalTopBarsHeight =
          parseInt(mob_header_height) + parseInt(admin_bar_height);
      }
      ...
      if (header_mobile.length && admin_bar.length && top_bar.length) {
        totalTopBarsHeight =
          parseInt(mob_header_height) +
          parseInt(admin_bar_height) +
          parseInt(top_bar_height);
    ...
    if (getWindowWidth() > 768) {
      var image_url = $('.banner-parallax-auto .banner-inner').css(
        'background-image'
      );

      if (image_url != 'none') {
        var bg_height =
          (scree_fix_auto.width() * bg_image_size('height', image_url)) /
          bg_image_size('width', image_url);
  ...
  search_mobile_trigger.on('click', function () {
    ...
    if (mobile_trigger_this.hasClass('active')) {
      mobile_trigger_this
        .closest('.advanced-search-mobile')
        .find('.advance-fields')
        .slideDown();
    } else {
      mobile_trigger_this
  ...
  var all_slider = $(
    '#banner-slider, .carousel, .lightbox-slide, .property-widget-slider'
  );
  ...
  var np_post_card = $('#carousel-post-card1');

  if (np_post_card.length > 0) {
    np_post_card.owlCarousel({
      rtl: houzez_rtl,
    ...
    $('.btn-crl-post-card-prev').on('click', function () {
      np_post_card.trigger('prev.owl.carousel', [600]);
    });

    $('.btn-crl-post-card-next').on('click', function () {
      np_post_card.trigger('next.owl.carousel', [600]);
    });
  }
  ...
  $('.prop_featured').on('change', function () {
    ...
    var price_featured = parseFloat(
      parent.find('.submission_featured_price').text()
    );
  ...
  function element_hide(ele, ele_class) {
    $(document).on('mouseup', function (e) {
      ...
      if (
        !nav_trigger.is(e.target) && // if the target of the click isn't the container...
