...
class ServicesReceipt
{
  private $services_payment_id;
  ...
  public function __construct($services_payment_id)
  {
    ...
    $this->services_payment_id = $services_payment_id;

    $info = $DB_payment->get_services_payment_info($services_payment_id);
    ...
    $this->service_type = $info['service_type'];
    $this->service_count = $info['services_count'];
    $this->service_price = $info['service_price'];

    $this->payment_amount = $info['payment_amount'];
    $this->C_F_amount = $info['convenience_fee'];
    $this->total_amount = $info['total_amount'];
    $this->payment_time = $info['payment_time'];

    $payment_type_info = $DB_payment->get_services_payment_method_type($services_payment_id);
  ...
  function download_receipt()
  {
    ...
    $reference_no = 'KJJ-' . $this->services_payment_id;
    $receipt->SetX($x + 15);
    $receipt->Cell(45, 10, "Reference No.", 1, 0, 'C');
    $receipt->Cell(110, 10, $reference_no, 1, 1, 'C');

    $receipt->SetX($x + 15);
    $receipt->Cell(45, 10, "Service Type", 1, 0, 'C');
    $receipt->Cell(110, 10, $this->service_type, 1, 1, 'C');

    $receipt->SetX($x + 15);
    $receipt->Cell(45, 10, "Service Price", 1, 0, 'C');
    $receipt->Cell(110, 10, '$ ' . $this->service_price . 'CAD', 1, 1, 'C');

    $receipt->SetX($x + 15);
    $receipt->Cell(45, 10, "Service Count", 1, 0, 'C');
    $receipt->Cell(110, 10, $this->service_count, 1, 1, 'C');
        ...
        <table cellspacing="0" cellpadding="0" style="background-color:white;">
          ...
          <tr style="background-color:white;">
            <td style="padding:0px 20px 10px 20px;">
              <table width="270" align="left" cellspacing="0" cellpadding="0" style="width: 600px;padding:0px 0 0px 0;">
                ...
                <tr><td style=" width: 5cm; border-bottom: 1px solid;border-left: 1px solid;padding: 10px;text-align: center;">Service Type</td><td style="border-bottom: 1px solid;border-left: 1px solid;padding: 10px;text-align: center; border-right: 1px solid;">' . $this->service_type . '</td></tr>
                <tr><td style=" width: 5cm; border-bottom: 1px solid;border-left: 1px solid;padding: 10px;text-align: center;">Service Price</td><td style="border-bottom: 1px solid;border-left: 1px solid;padding: 10px;text-align: center; border-right: 1px solid;">' . $this->service_price . '</td></tr>
                <tr><td style=" width: 5cm; border-bottom: 1px solid;border-left: 1px solid;padding: 10px;text-align: center;">Service Count</td><td style="border-bottom: 1px solid;border-left: 1px solid;padding: 10px;text-align: center; border-right: 1px solid;">' . $this->service_count . '</td></tr>
          ...
          <tr style="background-color:white;">
            <td style="padding:30px 20px 20px 20px;">
              <div align="center" style="text-align:center;margin:40px 0 20px 0;"><font face="Arial,sans-serif" size="2" color="#3DA9F5" style="font-family: Arial, sans-serif, serif, EmojiFont;"><span style="font-size:15px;"><a href="https://www.spgmanagement.com/admin/custom/invoice_receipt/invoice_receipt_controller.php?download_services_receipt&services_payment_id=' . $this->services_payment_id . '" target="_blank" rel="noopener noreferrer" style="text-decoration:none;"><font color="white"><span style="background-color:#80C246;padding:15px 45px;"><b>Download Receipt</b></span></font></a> </span></font></div>
            </td>
          </tr>
