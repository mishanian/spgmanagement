...
$apartment_lease = $_POST['apartment_lease'];
...
$pay_method_other="";
if(isChecked_checkboxes($pay_method,"0"))
...
########### page1 ##########
$pdf->AddPage();
$pdf->Image('imgs/lease_top.png',$pdf->GetX(),0,195,35,'PNG');
...
$pdf->SetFont('Times',"",9.2);
$pdf->Cell(173,6,$apartment_name.' '.$address_lease,"B",1,'L');
...
$x=$pdf->GetX();
$y=$pdf->GetY();
if(isChecked_checkboxes($pay_method,"1"))
    $pdf->Rect($x+2,$y+1,3,3,'F');
else
    $pdf->Rect($x+2,$y+1,3,3);

if(isChecked_checkboxes($pay_method,"2"))
    $pdf->Rect($x+22,$y+1,3,3,'F');
else
    $pdf->Rect($x+22,$y+1,3,3);

if(isChecked_checkboxes($pay_method,"3"))
...
$DB_services->add_lease_sign_request($user_id, $current_time,$name_lessee1,$email_lessee1,$telephone_lessee1,$address_lease,$apartment_name);

//switch page
header("Location: ../../service_approve.php?service_type=lease_signing");
...
<?php
//function to check whether a checkbox is checked or not.
function isChecked_checkboxes($checkbox_grp,$value){
    if(!empty($checkbox_grp)){
        foreach ($checkbox_grp as $selected){
            if($selected==$value)
                return true;
        }
    }
