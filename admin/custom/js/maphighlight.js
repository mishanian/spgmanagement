				...
				if(!area_options.neverOn && !area_options.alwaysOn) {
					...
					if(!has_canvas) {
                        ...
                        .bind('click',function(e){
                            ...
                            var dataId = $(area).attr("data-fid");

                            if(!dataId){
                                return;
                            }

                            $("#floor-image-map").empty();
                            $("#floor_mapper").attr("src","");

                            // AJAX call to get the Floor Plan Image
                             $.ajax({
                                    type:'GET',
                                    url:'custom/mapper_fetchmap.php',
                                    data:{
                                        data:dataId,
                                        level:"fid"
                                    },
                                    success:function(response){
                                        if(response){
                                            ...
                                            var imagePath = response.picture;
                                            var mapCoords = response.data;

                                            for(var areaCoord in mapCoords){
                                                var data = mapCoords[areaCoord]["mapping_area"];
                                                var areaDiv = "<area data-uid='"+ data[4] +"' target='' alt='' name='' title='"+data[2]+"' href='" + data[3] + "' coords='"+data[0]+"' shape='" + data[1] + "'>";
                                                $("#floor-image-map").append(areaDiv);
                                            }

                                            $("#floor_mapper").attr("src",imagePath);
                                            $("#floor_mapper").maphilight();
