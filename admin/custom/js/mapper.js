        ...
        d = {
            defaults: {
                ...
                input: {
                    ...
                    floor: {
                        selector: 'select[name^="im["][name$="][floor]"]',
                        fn: function() {
                            return a(this).attr("name").match(/im\[([\d]+)\]\[([^\)]+)\]/).slice(1)
                        }
                    },
                    ...
                    level: {
                        selector: 'input[name^="im["][name$="][level]"]',
                        fn: function() {
                            return a(this).attr("name").match(/im\[([\d]+)\]\[([^\)]+)\]/).slice(1)
                        }
                    },
            ...
            asHTML: function(b) {
                ...
                a.each(c[b].area, function(d, e) {
                    ...
                    coordinatesForDb = f.join(",");
                                        
                    var i = a("<area />");
                    
                    i.attr("target", c[b].area[d].target), i.attr("alt", c[b].area[d].title), i.attr("title", c[b].area[d].title), i.attr("level", c[b].area[d].level), i.attr("href", c[b].area[d].href), i.attr("coords", f.join(",")), i.attr("shape", c[b].area[d].shape), g.push(i[0].outerHTML)
                }), f.append("\n    " + g.join("\n    ") + "\n");
...
function(a) {
    a(document).on("init", function() {
        // Beaver Custom Code to get the Query String Parameters and load the image accordingly
        var urlParams = new URLSearchParams(window.location.search);
        
        var levelValue = urlParams.get("level"); // Level value for the page 
        
        if(levelValue != null){
             switch(levelValue){
                case 'fid':
                    var dataId = urlParams.get("fid");   
                    
                    break;
                case 'bid':
                    var dataId = urlParams.get("bid");
                    break;
                case 'uid':
                    var dataId = urlParams.get("uid");                    
                    break;
            }
        }
        
        // TO fetch the image from the database based on the level 
        jQuery.ajax({
               type:'GET',
               url:'mapper_fetchimg.php',
               data:{
                   data:dataId,
                   level:levelValue
               },                  
               success:function(response){
                   if(response){
                       // response has the image path 
                       var extension = response.substr(response.lastIndexOf('.') + 1);
                       var fileType = "";
                       
                       jQuery(".image-mapper-img").attr("src",response).addClass("img-responsive");                       
        ...
        a(document).on("click", "button.add-row", function(b) {
            ...
            var ab = a("#image-map").imageMapper("getData");
            
            a("#modal-code-result").text(b).css("whiteSpace", "pre")
        }),a(document).on("click", "#submitRow", function() { // CUSTOM CODE BEAVER 
            var ab = a("#image-map").imageMapper("getData"); // All values of the MAP rows that are added
            var levelValues = {level:jQuery("#levelValue").html()};
            
            switch(levelValues.level){
                case 'fid':
                    levelValues.fid = jQuery("#fidValue").html();
                    levelValues.bid = jQuery("#bidValue").html();
                    break;
                case 'bid':
                    levelValues.bid = jQuery("#bidValue").html();
                    break;
                case 'uid':
                    levelValues.fid = jQuery("#fidValue").html();
                    levelValues.bid = jQuery("#bidValue").html();
                    levelValues.uid = jQuery("#uidValue").html();
                    break;
            }
            
            // When submitted - store the values to the database as JSON
            jQuery.ajax({
               type:'POST',
               url:'mapper_save.php',
               data:{
                   data:JSON.stringify(ab),
                   level:JSON.stringify(levelValues)
               },
               dataType:'json',
               success:function(response){
                   if(response){
                       alert("Map Areas Saved");
                   }
               }
            });
