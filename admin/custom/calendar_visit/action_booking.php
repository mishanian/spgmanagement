	...
	public function get_availablility_dates($event_id){
		...
		foreach ( $availabilities as $row ) {
			...
			if($availability_type == 'monday') {
				...
				if($this_thursday >= $today && !in_array($this_thursday, $dates)) {
					$dates[] = $this_thursday;
				}
				...
				if($this_friday >= $today && !in_array($this_friday, $dates)) {
					$dates[] = $this_friday;
				}
...
//					}
		...
		usort($dates, function($date1, $date2) {       //sort the date
			$d1 = strtotime($date1);
			$d2 = strtotime($date2);
			return $d1 - $d2;
		});
	...
	private function get_time_ranges($event_id, $book_event_date) {
		...
		foreach ( $results as $row ) {
			$availability_type = $row['availability_type']; 
			if($availability_type == strtolower($dayofweek)) {  //omit
				...
				$specific_date = $row['availability_specific_date'];
				if(strtotime($specific_date) - strtotime($book_event_date) == 0) {  //the same date
					$start = $row['availability_start'];
					$end = $row['availability_end'];
					$time_ranges[] = array("start" => $start, "end" => $end);
				}
